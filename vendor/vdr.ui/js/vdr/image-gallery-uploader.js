$(document).ready(function() {
    !verboseBuild || console.log('-- starting vdr.imageGallery build');
...
vdr.imageGallery = {
	build: function () {
	    ...
	    var dropZoneTemplate = $.get('./gallery-dropzone-template.html', function(template) {
	    	vdr.imageGallery.makeDropzone(template); // Make Dropzone after loading template html
	    })
	    .fail(function() {
	        alert( "Image Gallery Error: could not load gallery html template" );
	    });

		!verboseBuild || console.log('            vdr.imageGallery build DONE');

	},
	events: function () {
		!verboseBuild || console.log('            vdr.imageGallery binding events');
	...
	makeDropzone: function (template) {
		!verboseBuild || console.log('            vdr.imageGallery.makeDropzone()');
