$(document).ready(function() {
    !verboseBuild || console.log('-- starting vdr.graphsStats build');
    
    vdr.graphsStats.build();
});

vdr.graphsStats = {
	build: function () {
		// Morris Charts
		!(Morris && $('.graph').length) || vdr.graphsStats.drawCharts();
		!($('.sparkspan').length) || vdr.graphsStats.sparklines();

		setTimeout(function() {
			vdr.graphsStats.liveStats();
		}, 2000);

		!verboseBuild || console.log('            vdr.graphsStats build DONE');
	},
	...
	statChange : false,
	liveStats : function () {
		if (vdr.graphsStats.statChange){
			clearTimeout(vdr.graphsStats.statChange);
		}
		var likeOrBuy = vdr.graphsStats.randomNum(0,5);
		if (likeOrBuy < 4)
			$('#like-count').text(parseInt($('#like-count').text()) + 1);
		else
			$('#buy-count').text(parseInt($('#buy-count').text()) + 1);

		vdr.graphsStats.statChange = setTimeout(vdr.graphsStats.liveStats, 500 * vdr.graphsStats.randomNum(1,4));
	},
	graph : {},
	redrawCharts : function () {
		!verboseBuild || console.log('            vdr.graphsStats.redrawCharts()');

		$.each(vdr.graphsStats.graph, function(index, val) {
			this.redraw();
		});
	},
	drawCharts : function () {
		!verboseBuild || console.log('            vdr.graphsStats.drawCharts()');

		if($('#myfirstchart').length)
		vdr.graphsStats.graph.Line = Morris.Line({
		...
		vdr.graphsStats.graph.Donut = Morris.Donut({
		    element: 'hero-donut',
		    data: [
		      {label: 'Jam', value: 25 },
		...
		if($('#hero-bar').length)
		vdr.graphsStats.graph.Bar = Morris.Bar({
		    element: 'hero-bar',
		    data: [
		...
		if($('#hero-area').length)
		vdr.graphsStats.graph.Area = Morris.Area({
		    element: 'hero-area',
		    data: [
	...
	sparklines: function () {
		!verboseBuild || console.log('            vdr.graphsStats.sparklines()');
